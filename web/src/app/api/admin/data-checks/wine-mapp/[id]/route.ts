...
/** GET: single winery name fix by id */
export async function GET(
  ...
  try {
    ...
    const rows = await query<WineMappRow>(
      `SELECT id, oldvworkname, newvworkname, created_at::text
       FROM tbl_wine_mapp WHERE id = $1`,
      [idNum]
    );
...
export async function PUT(
  ...
  try {
    ...
    const n = await execute(
      `UPDATE tbl_wine_mapp SET oldvworkname = $1, newvworkname = $2 WHERE id = $3`,
...
export async function DELETE(
  ...
  try {
    ...
    const n = await execute(`DELETE FROM tbl_wine_mapp WHERE id = $1`, [idNum]);
    if (n === 0) return NextResponse.json({ error: 'Not found' }, { status: 404 });
    return NextResponse.json({ ok: true });
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
